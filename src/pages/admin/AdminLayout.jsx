...
import { Layout, Menu, Button, Avatar, Badge, ConfigProvider, Tooltip, Grid, Drawer } from "antd";
import {
  DashboardOutlined,
  UserOutlined,
  ShopOutlined,
...
const AdminLayout = () => {
  ...
  const SidebarContent = () => (
    <>
      <div style={{ height: 90, display: "flex", alignItems: "center", paddingLeft: collapsed && screens.lg ? 24 : 28, gap: 12 }}>
        <div style={{
          width: 40, height: 40, background: COLORS.gold, borderRadius: 12, 
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: `0 0 20px ${COLORS.gold}40`, flexShrink: 0
        }}>
          <GlobalOutlined style={{ fontSize: 22, color: '#052018' }} />
        </div>
        {(!collapsed || !screens.lg) && (
          <span style={{ color: COLORS.textWhite, fontWeight: "700", fontSize: 20, letterSpacing: 0.5 }}>
            QingzhenMu
          </span>
        )}
      </div>
  ...
  return (
    ...
    >
      <Layout style={{ height: "100vh", background: COLORS.bgGradient, overflow: "hidden" }}>
        ...
        {!screens.lg && (
          <Drawer
            ...
            open={!collapsed}
            styles={{ body: { padding: 0, background: COLORS.sidebarBg, backdropFilter: "blur(20px)" } }}
            width={260}
            closeIcon={null}
          >
            <SidebarContent />
        ...
        <Layout style={{ background: "transparent" }}>
          ...
          >
            <Button
              ...
              style={{ fontSize: "20px", color: COLORS.textWhite, width: 40, height: 40 }}
            />

            <div style={{ display: "flex", alignItems: "center", gap: screens.xs ? 12 : 20 }}>
              {/* Notifikasi */}
              <Badge dot color={COLORS.gold} offset={[-6, 6]}>
                 <Button shape="circle" type="text" icon={<BellFilled style={{ color: COLORS.textWhite, fontSize: 20 }} />} />
              </Badge>
              ...
              <div style={{ 
                 ...
                 <Avatar size="large" style={{ backgroundColor: COLORS.gold, color: '#052018', fontWeight: 'bold' }}>A</Avatar>
                 {/* Sembunyikan nama di layar HP sangat kecil */}
                 {screens.sm && (
                   <div style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.1 }}>
                      <span style={{ color: "white", fontWeight: "600", fontSize: 13 }}>Admin</span>
                   </div>
                 )}
              </div>
